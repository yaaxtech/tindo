...
export const GET = rotaApi('GET /api/ai/sugerir-tarefas', async (_request: NextRequest) => {
...
export const POST = rotaApi('POST /api/ai/sugerir-tarefas', async (_request: NextRequest) => {
  const admin = getAdminClient();
  const usuarioId = await getUsuarioIdMVP();
