...
export const POST = rotaApi('POST /api/recalibrar/aplicar', async (request: NextRequest) => {
  const body = (await request.json()) as RequestBody;
  const { amostras: amostrasInput } = body;

  if (!Array.isArray(amostrasInput) || amostrasInput.length < 3) {
    throw new ErroValidacao('São necessárias pelo menos 3 avaliações para recalibrar.');
  }
  ...
  if (amostrasCompletas.length < 3) {
    throw new ErroValidacao('Tarefas não encontradas para calcular novos pesos.');
  }
