...
test('migration keeps the common author and experiment, removes the second route, and is idempotent', () => {
  const original = {
    ...
    contas: { preservar: true },
    terrenos: {
      rotina: {
        modelo: 'sol',
        effort: 'low',
        fallback_por_motivo: { quota_openai: [{ modelo: 'sonnet', effort: 'medium' }] },
      },
    },
    codex: {
      outra_config: true,
      terrenos: {
        rotina: { modelo: 'luna', effort: 'max', experimento: { id: 'medido', ativo: true } },
      },
    },
  };
  ...
  assert.deepEqual(next.terrenos.rotina.fallback_por_motivo.indisponivel_anthropic, [
    { modelo: 'sol', effort: 'low' },
  ]);
